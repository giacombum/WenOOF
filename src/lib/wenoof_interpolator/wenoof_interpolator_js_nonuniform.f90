module wenoof_interpolator_js_nonuniform
!-----------------------------------------------------------------------------------------------------------------------------------
!< Concrete WENO Jiang-Shu upwind interpolator for non-uniform grids object.
!-----------------------------------------------------------------------------------------------------------------------------------
...
use wenoof_optimal_weights_js_nonuniform
use wenoof_smoothness_indicators_js_nonuniform
...
private
save
public :: wenoof_interpolator_upwind_nonuniform, wenoof_constructor_upwind_nonuniform
!-----------------------------------------------------------------------------------------------------------------------------------

!-----------------------------------------------------------------------------------------------------------------------------------
type, extends(wenoof_constructor) :: wenoof_constructor_upwind_nonuniform
  !< Upwind biased WENO interpolator constructor for non-uniform grids,
  !<
  !< @note The constructed WENO interpolator implements the interpolator defined in *Grid adaptation with WENO schemes
  !< for non-uniform grids to solve convection-dominated partial differential equations*, J. Smit, M. van Sint Annaland,
  !< J. A. M. Kuipers, Chemical Engineering Science, vol. 60, issue 10, pp. 2609--2619, doi:10.1016/j.ces.2004.12.017
  !< and in *Efficient implementation of WENO schemes to nonuniform meshes*, Nelida Črnjarić-Žic, Senka Maćešić, Bojan Crnković,
  !< ANNALI DELL'UNIVERSITA' DI FERRARA, 2007, vol. 57, issue 2, pp. 199--215, doi:10.1007/s11565-007-0013-1
...
endtype wenoof_constructor_upwind_nonuniform

interface wenoof_constructor_upwind
  procedure wenoof_constructor_upwind_nonuniform_init
endinterface

type, extends(wenoof_interpolator) :: wenoof_interpolator_upwind_nonuniform
  ...
  contains
    ...
    ! private methods
    final :: finalize
endtype wenoof_interpolator_upwind_nonuniform
!-----------------------------------------------------------------------------------------------------------------------------------
contains
  ! constructor_upwind
  elemental function wenoof_constructor_upwind_nonuniform_init(S, eps) result(constructor)
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  real(R_P),    intent(IN), optional         :: eps         !< Parameter for avoiding divided by zero when computing IS.
  type(wenoof_constructor_upwind_nonuniform) :: constructor !<WENO constructor.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  return
  !---------------------------------------------------------------------------------------------------------------------------------
  endfunction  wenoof_constructor_upwind_nonuniform_init
  ...
  type is(wenoof_constructor_upwind_nonuniform)
    call self%destroy
  ...
  !< Finalize object.
  !---------------------------------------------------------------------------------------------------------------------------------
  type(wenoof_interpolator_upwind_nonuniform), intent(INOUT) :: self !< WENO interpolator.
  !---------------------------------------------------------------------------------------------------------------------------------
...
endmodule wenoof_interpolator_js_nonuniform
